...
bool test_createGaussianKernel1D(void)
{
   ...
   if (k.rows != 1){
      cout << "ERROR: Dip3::createGaussianKernel1D(): Wrong size! row" << endl;
      return false;
   }
   if (k.cols != 11){
      cout << "ERROR: Dip3::createGaussianKernel1D(): Wrong size! col" << endl;
      return false;
   }
...
bool test_circShift(void)
{
    ...
    cout << "Message: Dip3::test_circShift() seems to be correct" << endl;
    return true;
}
